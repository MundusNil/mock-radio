...
**案头**:
曲目开播时的检索笔记（作品场景 / 玩家社区 / 乐评）。占用音乐时间预取，LangGraph 多轮（搜→质检→不足补搜，整图挂钟预算 90s）；检索默认走本地管道（SearXNG 查询→抓正文→普通 token 提炼，来源 URL 可回查），关 `deskLocalSearch` 回方舟 web_search（贵 token）；开口只从笔记写，不再联网。不是 L1 记忆，不是世界书。
